...
SPACESHIP_KUBENAMESPACE_PREFIX="${SPACESHIP_KUBENAMESPACE_PREFIX:="at "}"
SPACESHIP_KUBENAMESPACE_SUFFIX="${SPACESHIP_KUBENAMESPACE_SUFFIX:="$SPACESHIP_PROMPT_DEFAULT_SUFFIX"}"
SPACESHIP_KUBENAMESPACE_SYMBOL="${SPACESHIP_KUBENAMESPACE_SYMBOL:="☸️ "}"
...
spaceship_kubenamespace() {
  [[ $SPACESHIP_KUBENAMESPACE_SHOW == false ]] && return

  local kube_context=$(kubectl config get-contexts "$(kubectl config current-context)" | awk "/$(kubectl config current-context)/ {print \$5}")

  _prompt_section \
    "$SPACESHIP_KUBENAMESPACE_COLOR" \
    "$SPACESHIP_KUBENAMESPACE_PREFIX" \
    "${SPACESHIP_KUBENAMESPACE_SYMBOL}${kube_context}" \
